...
if selected_files:
    ...
    try:
        ...
        with tab_overview:
            ...
            patient_word = get_patient_word(total_calls)

            activity_level = "высокий уровень" if percent_any > 70 else "средний уровень" if percent_any > 50 else "низкий уровень"
            satisfaction_level = ("высокий уровень" if avg_csi > 8 else "удовлетворительный уровень" if avg_csi > 6 else "низкий уровень") if avg_csi else "отсутствует"
            max_csi = dept_stats['средний_CSI'].max()
            min_csi = dept_stats['средний_CSI'].min()
            max_csi_dept = dept_stats['средний_CSI'].idxmax()
            min_csi_dept = dept_stats['средний_CSI'].idxmin()
            max_calls = dept_stats['звонков'].max()
            max_calls_dept = dept_stats['звонков'].idxmax()
            participation_rec = f'Увеличить долю пациентов, отвечающих на все вопросы с {percent_all:.1f}% до 80-90%.' if percent_all < 80 else 'Вовлеченность достаточная, поддерживать на достигнутом уровне.'
            low_csi_depts = ", ".join(dept_stats[dept_stats['средний_CSI'] < avg_csi].index.tolist()) if avg_csi else "нет"

            # Текстовая сводка для доклада
            summary = f"""
<strong>Общий анализ опроса пациентов:</strong><br><br>
- <strong>Объем данных:</strong> Проанализированы данные о {total_calls:,} {patient_word}.<br>
- <strong>Активность респондентов:</strong> {percent_any:.1f}% пациентов ответили хотя бы на один вопрос ({any_answered:,} чел.), что указывает на {activity_level} вовлеченности.<br>
- <strong>Полнота ответов:</strong> Лишь {percent_all:.1f}% пациентов ответили на все {len(question_cols)} вопроса ({all_answered:,} чел.), что составляет {avg_answers_with_some:.1f} ответа в среднем на человека участвовавшего в опросе.<br>
- <strong>Индекс удовлетворенности клиента (CSI):</strong> Среднее значение CSI составляет {avg_csi:.1f} балла по 10-балльной шкале, что расценивается как {satisfaction_level} удовлетворенности.<br><br>
<strong>Анализ по отделениям:</strong><br><br>
Данные доступны во вкладке "Отделения". Основные выводы:<br>
- Максимальный CSI: {max_csi:.1f} балла (отделение {max_csi_dept})<br>
- Минимальный CSI: {min_csi:.1f} балла (отделение {min_csi_dept})<br>
- Наибольшее число обзвонов: {max_calls} (отделение {max_calls_dept})<br><br>
<strong>Рекомендации по улучшению:</strong><br><br>
1. <strong>Повышение вовлеченности:</strong> {participation_rec}<br>
2. <strong>Фокус на отделениях:</strong> Обратить внимание на отделения с низким CSI ({low_csi_depts}) для улучшения качества обслуживания.<br>
3. <strong>Мониторинг трендов:</strong> Использовать вкладку "Сравнения" для отслеживания динамики CSI по периодам.<br><br>
<em>Отчет подготовлен автоматически на основе загруженных данных.</em>
            """

            # Форматированный вывод сводки
